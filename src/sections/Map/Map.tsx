...
import "./Map.scss";
import { locations } from "../../components/MapComponent/MapComponent";

const Map = () => {
  const columnSize = Math.ceil(locations.length / 3);
  const columns = [locations.slice(0, columnSize), locations.slice(columnSize, columnSize * 2), locations.slice(columnSize * 2)];
    ...
    <section className="map">
      <div className="row">
        ...
        <div className="col-xl-6 ps-3 d-none d-xl-block py-5">
          <h2 className="section-title ms-0 text-start mb-2 mt-2">Were we do</h2>
